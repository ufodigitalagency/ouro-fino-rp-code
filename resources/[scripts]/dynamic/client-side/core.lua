...
RegisterCommand("PlayerFunctions",function()
	...
	if not Dynamic and not IsPauseMenuActive() and not LocalPlayer.state.Banned and not LocalPlayer.state.Commands and not LocalPlayer.state.Handcuff and not LocalPlayer.state.Prison and GetEntityHealth(Ped) > 100 then
		...
		for Permission,v in pairs(Groups) do
			if Permission ~= "Admin" and not v.Block and LocalPlayer.state[Permission] then
				if Painels == 0 then
		...
		if parseInt(LocalPlayer.state.Passport) == 1 then
			exports.dynamic:AddMenu("Central Administrativa","Ferramentas do dono do servidor.","ownerAdmin")
			exports.dynamic:AddButton("Abrir Painel Admin","Abre a central administrativa geral.","af_owner_panel:dynamicAction","openPanel","ownerAdmin",true)
		end
...
RegisterKeyMapping("EmergencyFunctions","Abrir menu de servico e emergencia.","keyboard","F10")
