	...
	while Actived do
		...
		if EntityHit and GetEntityType(Entitys) ~= 0 then
			...
			elseif IsEntityAPed(Entitys) and not DecorGetBool(Entitys,"CREATIVE_PED") and not IsPedAPlayer(Entitys) and #(Coords - HitCoords) <= 2.0 then
				local PedNetwork = NetworkGetEntityIsNetworked(Entitys) and NetworkGetNetworkIdFromEntity(Entitys) or 0
				Selected = { PedNetwork,Entitys }

				if LocalPlayer.state.Admin then
					table.insert(Menu,{ event = "DeletePed", label = "Deletar", tunnel = "paramedic" })
				end

				local streetSalesStarted = GetResourceState("sao_judas_street_sales") == "started"
				local canOffer = false
				if streetSalesStarted then
					local ok,result = pcall(function()
						return exports.sao_judas_street_sales:CanOffer(Entitys)
					end)
					canOffer = ok and result == true
				end

				if canOffer then
					table.insert(Menu,{ event = "sao_judas_street_sales:Offer", label = "OFERECER PRODUTO", tunnel = "client" })
				end
