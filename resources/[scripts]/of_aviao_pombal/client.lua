...
local vSERVER = Tunnel.getInterface("of_aviao_pombal")
...
local function removeDeliveryTarget()
    if DeliveryTargetRegistered and GetResourceState("target") == "started" then
        exports.target:RemCircleZone("AviaoPombal:Delivery")
    end
...
local function startTargetOptions()
    ...
    return {
        {
            event = "of_aviao_pombal:ToggleService",
            tunnel = "client",
            label = routeLabel
        },{
...
local function refreshStartTargetOptions()
    if StartTargetRegistered and GetResourceState("target") == "started" then
        exports.target:LabelOptions("AviaoPombal:Start",startTargetOptions())
    end
end
...
local function createStartNpc()
    ...
    if not model then
        ...
        print("[of_aviao_pombal] Não foi possível carregar o modelo do NPC inicial.")
        return false
    end
...
local function registerStartTarget()
    ...
    exports.target:AddCircleZone("AviaoPombal:Start",vector3(Config.Start.x,Config.Start.y,Config.Start.z),1.1,{
        name = "AviaoPombal:Start",
...
local function createCustomer(destinationId)
    ...
    if not model then
        ...
        print(("[of_aviao_pombal] Não foi possível carregar o cliente %s."):format(tostring(modelName)))
    ...
    if GetResourceState("target") == "started" then
        exports.target:AddCircleZone("AviaoPombal:Delivery",vector3(destination.x,destination.y,destination.z),Config.Delivery.TargetRadius,{
            name = "AviaoPombal:Delivery",
...
local function setReturnToBase()
    ...
    CurrentCustomerModel = nil
    AwaitingReturn = true
    createRouteBlip(Config.Start,"Retornar ao Pombal")
    refreshStartTargetOptions()
end
...
AddEventHandler("of_aviao_pombal:ToggleService",function()
    if Busy then
        return
    end
...
AddEventHandler("of_aviao_pombal:Deliver",function()
...
RegisterNetEvent("of_aviao_pombal:ForceCleanup",function(_,message)
    cleanupRoute()
...
AddEventHandler("onResourceStop",function(resourceName)
    ...
    if GetResourceState("target") == "started" then
        if StartTargetRegistered then
            exports.target:RemCircleZone("AviaoPombal:Start")
        end
