...
Tunnel.bindInterface("of_aviao_pombal",API)
...
local function cooldownKey(Passport)
    return "AviaoPombal:Cooldown:"..tostring(Passport)
end
...
local function terminateSession(playerSource,reason,message)
    ...
    TriggerClientEvent("of_aviao_pombal:ForceCleanup",playerSource,reason,message)
end
...
function API.ToggleService()
    ...
    if not hasMembership(Passport) then
        return response(false,"membership_required","Somente membros do Pombal podem acessar esta rota.")
    end
...
function API.BeginDelivery(destinationId)
    ...
    if not hasMembership(Passport) or not isOnDuty(Passport) then
        terminateSession(playerSource,"service_lost","A rota foi encerrada porque você saiu do serviço do Pombal.")
...
function API.CompleteDelivery(destinationId,token)
    ...
    local split = exports.pombal_finance:SplitRevenue("airplane",Passport,amount,"airplane:"..pending.Token)
    if not split or not split.Success then
        return response(false,"payment_unavailable","Nao foi possivel registrar a divisao financeira. Tente novamente.")
    end
    ...
    if session.Current > #session.Route then
        ...
        return response(true,"return_to_base","Entrega concluída. Retorne ao Pombal para finalizar.",{
            reward = amount,
            completed = completed,
            total = #session.Route,
            returnToBase = true
    ...
    while true do
        ...
        for playerSource,session in pairs(Sessions) do
            ...
            elseif not hasMembership(Passport) or not isOnDuty(Passport) then
                terminate[#terminate + 1] = { Source = playerSource, Reason = "service_lost", Message = "A rota foi encerrada porque você saiu do serviço do Pombal." }
            end
        end
